...
if ( isset( $_FILES[ 'arquivo' ][ 'name' ] ) && $_FILES[ 'arquivo' ][ 'error' ] == 0 ) {
    ...
    if ( strstr ( '.jpg;.jpeg;.png', $extensao ) ) {
        ...
        if ( @move_uploaded_file ( $arquivo_tmp, $destino ) ) {
			...
			$qsqlImagem = "Select logo from corretora where id='$id'";
			...
			$imagem = $destino;
			$qsql = "UPDATE `corretora` SET `logo`= '$imagem' WHERE id =$id";
			if($rs = mysqli_query($conn,$qsql)){
                ...
                echo "<script language='javaScript'>window.location.href='/pag/Corretoras'</script>";
	  	 		exit;
			}
			else{
