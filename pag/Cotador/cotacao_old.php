...
for ($i=0;$i<=$registros;$i++){
	...
	if (substr($reg,0,5)=='plano'){
		...
		$qsqlPlano = "SELECT planos.id, planos.cod_oper, planos.tipo_plano,planos.plano, 						operadora.nome_abrev 
						FROM `planos`,operadora 
						WHERE planos.cod_oper = operadora.id and 
							  planos.id = $codPlano";
		
		if($rdPlano = mysqli_query($conn,$qsqlPlano)){
			if($regPlano=mysqli_fetch_array($rdPlano)){
				$nomeAbrev = $regPlano['nome_abrev'];
				$codPlano  = $regPlano['id'];
				...
				if($regPlano['tipo_plano']=='PF'){
					
					$qsqlTabela = "Select * from tabela_planos as tabela where 
				...
				if($regPlano['cod_oper']<>$cod_operadora){
					...
					$rdTabela = mysqli_query($conn,$qsqlTabela);
					if($regTabela=mysqli_fetch_assoc($rdTabela)){
								echo "<h2 align=center><img src='../../img/logos/".$nomeAbrev.".png' alt='".$nomeAbrev."' style='height:80px'/></h2>";
								$titular = $regTabela['umTitular'];
								$compulsorio = $regTabela['compulsorio'];
						$valorTotal =   $faixa1  * $regTabela['faixa1'] +
									    $faixa2  * $regTabela['faixa2'] +
										$faixa3  * $regTabela['faixa3'] +
										$faixa4  * $regTabela['faixa4'] +
										$faixa5  * $regTabela['faixa5'] +
										$faixa6  * $regTabela['faixa6'] +
										$faixa7  * $regTabela['faixa7'] +
										$faixa8  * $regTabela['faixa8'] +
										$faixa9  * $regTabela['faixa9'] +
										$faixa10 * $regTabela['faixa10'];
?>					
						...
						if($compulsorio<>''){
							...
							<tr>
								...
								<td><?php if ($faixa1 <> 0){ 
									echo number_format($regTabela['faixa1'], 2, ',', '.');
														   }else{ echo " - ";					} ?></td>
								<td><?php if ($faixa2 <> 0){ 
									echo number_format($regTabela['faixa2'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa3 <> 0){ 
									echo number_format($regTabela['faixa3'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa4 <> 0){ 
									echo number_format($regTabela['faixa4'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa5 <> 0){ 
									echo number_format($regTabela['faixa5'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa6 <> 0){ 
									echo number_format($regTabela['faixa6'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa7 <> 0){ 
									echo number_format($regTabela['faixa7'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa8 <> 0){ 
									echo number_format($regTabela['faixa8'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa9 <> 0){ 
									echo number_format($regTabela['faixa9'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
								<td><?php if ($faixa10 <> 0){ 
									echo number_format($regTabela['faixa10'], 2, ',', '.');
														   }else{ echo " - ";					}  ?></td>
